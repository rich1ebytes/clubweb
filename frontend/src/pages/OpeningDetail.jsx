    ...
    useEffect(() => {
        const fetchOpening = async () => {
            try {
                ...
                setError(err.response?.data?.message || "Failed to fetch opening details. Please try again later.");
            } finally {
                setLoading(false);
            }
        };
        ...
        <div className="max-w-6xl mx-auto p-4 sm:p-6 lg:p-8 mt-8">
            <div className="bg-white rounded-lg shadow-2xl overflow-hidden flex flex-col md:flex-row">
                ...
                <div className="md:w-1/2 p-6 flex flex-col justify-between">
                    <div>
                        <h1 className="text-3xl font-bold text-gray-900 mb-2">{opening.role}</h1>
                        <p className="text-lg text-gray-600 mb-4">
                            For: <span className="font-semibold text-brand-primary">{opening.openingFor}</span>
                        </p>
                        <div className="text-md text-gray-700 mb-4">
                            <p className="mb-2">
                                <span className="font-semibold">Deadline:</span> {new Date(opening.deadline).toLocaleDateString()}
                            </p>
                        </div>
                        <p className="text-gray-800 text-base leading-relaxed mb-6">{opening.description}</p>
                    </div>
